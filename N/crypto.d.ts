...
interface crypto_ {
   ...
   * padding is PKCS#5.
   *
   * A caller-supplied `iv` property is silently accepted at construction but has no effect: the
   * cipher always generates its own random initialization vector, exposed on the `CipherPayload`
   * returned by `final`. There is no way to pin the iv when encrypting.
   *
...
declare namespace crypto_ {
  ...
  export interface Cipher {
     ...
     *
     * `final` may be called more than once on the same Cipher. Each call returns a new
     * `CipherPayload` with a freshly generated iv, re-encrypting the accumulated input; it does not
     * throw and does not repeat the previous payload.
